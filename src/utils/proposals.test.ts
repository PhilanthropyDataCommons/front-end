...
test('can select most preferred applicant name', () => {
	...
	expect(getPreferredApplicantNameValues(proposal)).toEqual(
...
test('can cascade through preferred applicant names', () => {
	...
	expect(getPreferredApplicantNameValues(leastPreferredProposal)).toEqual(
		leastPreferredNameValues,
	);
});
...
test('can fall back to fallback applicant name', () => {
	...
	expect(getPreferredApplicantNameValues(blankProposal)).toEqual([
		PROPOSAL_APPLICANT_NAME_FALLBACK,
	]);
});
