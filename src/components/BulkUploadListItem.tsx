...
export const BulkUploadListItem = ({ upload }: BulkUploadListItemProps) => {
  const {
    id, fileName, fileSize, status, createdAt,
  } = upload;
  ...
  const formattedFileSize = fileSize ? (
    <span title={`${fileSize.toLocaleString()} byte${fileSize !== 1 ? 's' : ''}`}>
  ...
  switch (status) {
    case BulkUploadStatuses.PENDING:
      ...
      metadata.push(relativeTimestamp, formattedFileSize, 'Waiting to process…');
      break;
  ...
  const metadataElements = metadata
    ...
    .reduce((accumulator, datum) => (accumulator === null ? datum : (
      <>
        {accumulator}
        {METADATA_DELIMITER}
        {datum}
