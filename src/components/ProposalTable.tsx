...
export const ProposalTable = ({ values, version }: ProposalTableProps) => {
		...
		<Table>
			<TableHead fixed>
				<TableRow>
					...
					<ColumnHead>{`Version ${version}`}</ColumnHead>
				</TableRow>
			</TableHead>
			<TableBody>
				{values.map(({ shortCode, fieldName, position, value }) => (
					<TableRow key={position}>
						<RowHead>
							{displayShortCodes ? (
								<code className="short-code">{shortCode}</code>
							) : (
								fieldName
							)}
						</RowHead>
