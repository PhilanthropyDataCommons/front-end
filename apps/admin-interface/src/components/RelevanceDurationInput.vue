...
const initializeFromModelValue = (): void => {
	...
	if (
		 ...
		 * Overall it is useful to avoid hard coding magic numbers, but in this case
		 * we have to compare using Luxon's default comparison operators,
		 * which expect numbers
		 */
		months >= Duration.fromObject({ months: 1 }).as('months') &&
		 ...
		 * which expect numbers
		 */
		days >= Duration.fromObject({ days: 1 }).as('days') &&
...
const updateValue = (): void => {
	if (selectedValue.value === 'permanent') {
		...
		/* eslint-disable-next-line @typescript-eslint/no-magic-numbers --
		 * Overall it is useful to avoid hard coding magic numbers, but in this case
		 * we have to compare using Luxon's default comparison operators, which expect numbers
		 */
		if (num > 0) {
